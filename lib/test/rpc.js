    ...
    var tests = {
        _call_methods: function(client, server, methods, done){
            ...
            for(var method in methods){
                ...
                client.call(method, args).then(function(result){
                    if(info.result)
                        assert.deepEqual(info.result, result);
                    else
                        throw new Error("got a response when we should have gotten an error for \""+method+"\"!");
        ...
        exposing_methods: function(c, s, done){
            ...
            s.exposeMethod("error.add", rpc_methods.async.add);
            s.exposeMethod("error.add_async", rpc_methods.async.add);
            s.exposeMethod("error.add_throws_async", rpc_methods.async.add);
        ...
        notifications: function(c, s, done){
            s.exposeMethod("testMethod", function(a, b){
                assert.equal(a, 2);
                assert.equal(b, 3);
                done();
            });
